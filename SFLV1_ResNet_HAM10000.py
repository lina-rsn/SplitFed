...
# Adapted from a SplitfedV1 ResNet18/HAM10000 implementation to use CIFAR-10
#
...
INTERMEDIATE_DIR = os.path.join(WD, "intermediate", INTERMEDIATE_DATA_DIR)
LABEL_DIR = os.path.join(WD, "labels", INTERMEDIATE_DATA_DIR)
Path(INTERMEDIATE_DIR).mkdir(parents=True, exist_ok=True)
Path(LABEL_DIR).mkdir(parents=True, exist_ok=True)
SAVE_INTERMEDIATE = True
IMAGE_DIR = os.path.join(WD, "images", INTERMEDIATE_DATA_DIR) 
...
class ResNet18_server_side(nn.Module):
    def __init__(self, block, num_layers, classes):
        ...
        # Adaptive pooling instead of a fixed kernel_size=7 AvgPool2d:
        # CIFAR-10 (32x32) produces a much smaller feature map by this point
        # than the original 64x64 HAM10000 input did, so a fixed 7x7 kernel
        
        # AdaptiveAvgPool2d(1) always reduces to 1x1 regardless of input size.
    ...
    def forward(self, x):
        ...
        x7 = F.adaptive_avg_pool2d(x6, (1, 1))
...
#                         Data loading
#=============================================================================
# CIFAR-10 is downloaded/loaded directly via torchvision -- no CSV metadata,
# no manual file-path indexing, and no custom SkinData class are needed
# the way they were for HAM10000.
data_dir = 'data'
